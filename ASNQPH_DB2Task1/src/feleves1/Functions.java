...
public class Functions {
	...
	private static int selection;
		...
		public static void insertConsoleKiadok(Connection conn) throws SQLException
		{
			int kiadoid=0;
			java.sql.Date date = null;
			String nev="",datum="";
			PreparedStatement prstmt = conn.prepareStatement("INSERT INTO KIADOK VALUES(?,?,?)");
			Scanner sc = new Scanner(System.in);
			//SimpleDateFormat myFormat = new SimpleDateFormat("dd/MM/yyyy");
			
				System.out.println("Kérem adja meg a kiado id-jét!");
				kiadoid = sc.nextInt();
				System.out.println("Kérem adja meg nevét!");
				nev = sc.next();
				System.out.println("Kérem adja meg a könyv kiadásának idejét! (yyyy-mm-dd)");
				datum = sc.next();
				System.out.println("A rekord felvive!\n");
				if(kiadoid != 0 && nev != "" && datum != "")
				{
				date = Date.valueOf(datum);
				prstmt.setInt(1, kiadoid);
		...
		public static void setNameOfKiadok(Connection conn,String nev) throws SQLException 
		{
			PreparedStatement prstmt = conn.prepareStatement("UPDATE KIADOK SET NEV = ? WHERE KIADOID = ?");
			Scanner sc = new Scanner(System.in);
			int id = 0;
			try {
				System.out.println("Adja meg az új nevet!");
				nev = sc.next();
				System.out.println("Adja meg a módosítandó ID-t!");
				id = sc.nextInt();
				if(id != 0 && nev != "" )
				{
					...
					prstmt.setInt(2, id);
					prstmt.executeUpdate();
					System.out.println("Kiado neve modositva!\n");
				}
				} catch(Exception e)
				{
					System.out.println("Nem megfelelõek az adatok");
		...
		public static void deleteByIdKiadok(Connection conn, int id) throws SQLException
		{
			
			PreparedStatement prstmt = conn.prepareStatement("DELETE KIADOK WHERE KIADOID=?");
			Scanner sc = new Scanner(System.in);
			try {
			...
			System.out.println("Deleted kiadok: "+ prstmt.executeUpdate());
			sc.close();
			prstmt.close();
		} // end deleteByIdKiadok
		...
		public static int menu() {
		    ...
		    System.out.println("12 - Kiadok tabla feltoltese (Console)");
		    System.out.println("13 - Torles kiadok tablabol ID alapjan");
		    System.out.println("14 - Kiadok tabla modositasa");
